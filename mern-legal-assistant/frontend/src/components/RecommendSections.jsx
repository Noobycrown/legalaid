...
const RecommendSections = () => {
  ...
  const handleRecommend = async () => {
    if (!inputText.trim()) {
      setError('Please enter case description');
      return;
    }
    ...
    setError('');
    try {
      const response = await axios.post('http://localhost:5000/api/ai/recommend-sections', {
        caseText: inputText,
      });
      ...
      setChatHistory(prev => [
        ...
        { type: 'ai', text: response.data.sections }
      ]);
      setInputText('');
    } catch (err) {
      const msg = err?.response?.data?.error || err.message;
      setError('Failed to recommend sections: ' + msg);
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const formatOutput = (text) => {
    ...
    return text
      ...
      .replace(/(<li>.*?<\/li>)+/gs, match => match.includes('1.') ? `<ol>${match}</ol>` : `<ul>${match}</ul>`);
  };

  return (
    <div className="tool-container" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="tool-header"><h3>Recommend Legal Sections</h3></div>
      ...
      <div className="chat-input-bar-modern">
        <label htmlFor="section-upload" className="chat-icon-button">➕</label>
        <input
          id="section-upload"
          type="file"
          accept=".pdf,.docx"
